...
int bx, cx, dx;
{
   ...
   /* TODO: Fork only needs to dup DS */

   proc->pending_op = 0;
   *newproc = *proc;
   newproc->ppid = newproc->pid;
   newproc->memory = __segalloc(proc->memsize);
   ...
   newproc->ss = newproc->ss-newproc->cs + newproc->memory;
   newproc->ds = newproc->ds-newproc->cs + newproc->memory;
   newproc->cs = newproc->memory;

   newproc->pid = nextpid++;

   /* Duplicate memory */
   for(i=0; i<newproc->memsize; i+=size )
   {
      size = 0xFFF; if(size > newproc->memsize-i) size = newproc->memsize-i;

      __movedata(proc->memory+i, 0, newproc->memory+i, 0, size*16);
   }
