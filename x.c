...
int argc; char **argv;
{
   ...
   prts("Cpu identifier - (C) R de Bath <rdebath@cix.compulink.co.uk>\n");
   ...
   {
      name[2] = "0123456789ABCDEF"[major&0xF];
      name[5] = (major>15?'+':'\0');
      prts(name);
      if(c&0x01) prts(" in protected mode");
      prts(" MSW= ");
      if( c&0x10 ) prts("ET,"); else prts("--,");
      if( c&0x08 ) prts("TS,"); else prts("--,");
      if( c&0x04 ) prts("EM,"); else prts("--,");
      if( c&0x02 ) prts("MP,"); else prts("--,");
      if( c&0x01 ) prts("PE\n"); else prts("--\n");

      if( !fpu && ( c&0x06) )
	 prts("An FPU appears to exist but it is unavailable\n");
      else
      {
	 if( c&0x02 ) prts("Math processor requires WAIT\n");
